...
const copyProfile = 'C:/Users/91779/tresor-chrome-profile-copy';
...
(async () => {
  console.log('Copying Chrome profile (this may take a moment)...');
  fs.rmSync(copyProfile, { recursive: true, force: true });
  copyRecursive(sourceProfile, copyProfile);
  console.log('Profile copied. Launching detached Chrome with CDP...');
  ...
  chrome.unref();
  console.log(`Chrome launched with CDP on port ${port} (pid ${chrome.pid}).`);
})();
