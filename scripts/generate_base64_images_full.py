...
img_dir = os.path.join(base, 'images-full')
out_path = os.path.join(base, 'images_full_base64.json')
max_kb = 900

with open(os.path.join(base, 'inventory_full.json'), encoding='utf-8') as f:
    rows = json.load(f)

output = {}
for row in rows:
    code = row['code']
    if not row['listing_price']:
        print(f'  X {code}: skipped (no price)')
        continue
    ...
    if not os.path.exists(local_path):
        print(f'  X {code}: image file missing {image_file}')
        continue
