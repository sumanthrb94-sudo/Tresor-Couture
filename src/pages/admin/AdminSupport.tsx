...
const orderLabel = (orderId?: string) => (orderId ? `TC-${orderId.slice(-8).toUpperCase()}` : '');
...
const ChatInbox: React.FC<{ conversations: ChatConversation[] }> = ({ conversations }) => {
  ...
  const filtered = useMemo(() => {
    ...
    return conversations.filter(c =>
      ...
      || orderLabel(c.orderId).toLowerCase().includes(q));
  }, [conversations, query]);
            ...
            filtered.map(c => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-[10px] font-bold uppercase tracking-wider text-[color:var(--color-myntra-pink)]">{orderLabel(c.orderId)}</p>
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-[13px] font-bold text-[color:var(--color-myntra-navy)] truncate">
              ...
              <span className="ml-2 text-[11px] font-bold text-[color:var(--color-myntra-pink)]">{orderLabel(conversation.orderId)}</span>
            </p>
