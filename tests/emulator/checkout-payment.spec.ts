 ...
 *
 * UPI and Card unlock purely from the presence of VITE_CASHFREE_MODE, so
 ...
 *
 * It deliberately does NOT complete a Cashfree payment — that needs the real
 * hosted modal and a real card.
 */
test('Checkout · Payment methods reflect Cashfree configuration', async ({ browser }: { browser: Browser }) => {
  ...
  const rec = new Recorder({
    ...
    purpose:
      'UPI and Card are enabled if and only if VITE_CASHFREE_MODE is present in the build. This is what makes going live a matter of setting an environment variable and redeploying, with no code change — and what stops the store offering a payment method it cannot actually take.',
    reproduce: [
      'Build the app with VITE_CASHFREE_MODE set (sandbox or production) and serve it.',
  ...
  try {
    ...
    if (configured) {
      ...
      rec.note('Cashfree configured → UPI and Card are selectable');
      ...
      await rec.step(cust, 'Cashfree not configured → UPI and Card stay disabled',
