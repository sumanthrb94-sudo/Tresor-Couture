...
const SECTIONS: Section[] = [
  ...
  { slug: 'admin-16-analytics', section: 'analytics', title: 'Admin · Views',
    purpose: 'The Views screen reports storefront traffic inside the admin, so the numbers do not require a Vercel login to read. It fetches them from /api/admin/analytics, which is a serverless function and therefore absent under the emulator preview server — so this run also proves the page degrades to its heading and a stated reason instead of a white screen when analytics cannot be reached.',
    heading: /^views$/i },
