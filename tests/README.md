...
- **`.github/workflows/ci.yml`** — runs on every push + PR: `npm ci` → `npm run lint` (tsc) → `npm run build` → a guard that fails the build if a server SDK/secret (`firebase-admin`, `CASHFREE_SECRET_KEY`, `BREVO_API_KEY`) leaks into `dist/assets/`. This is the green-gate for merging.
...
doubles as a deployment config check (did `BREVO_API_KEY` / Cashfree env take?).
