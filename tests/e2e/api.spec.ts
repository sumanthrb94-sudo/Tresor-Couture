 ...
 *
 * These endpoints require a CSRF double-submit cookie and an allowed Origin
 * header, so each test fetches /api/csrf first.
 */

const ORIGIN = process.env.BASE_URL || 'https://tresorcouture.in';

async function csrfHeaders(request: any) {
...
test.describe('POST /api/contact (Brevo capture)', () => {
  ...
  test('rejects a request with neither a valid email nor a phone', async ({ request }) => {
    const headers = await csrfHeaders(request);
    const res = await request.post('/api/contact', { headers, data: { source: 'e2e' } });
