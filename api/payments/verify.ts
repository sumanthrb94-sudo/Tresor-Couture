 ...
 *   order: {
 *     userId, items:[{fabricId, meters, color?}], couponCode?,
 ...
 *      is short, decrement inStockMeters, and write the order with
  ...
  try {
    ...
    await db.runTransaction(async (tx) => {
      ...
      for (let i = 0; i < breakdown.lines.length; i++) {
        ...
        const stock = data.inStockMeters;
        if (typeof stock === 'number') {
          if (stock < line.meters) {
            throw new Error(`insufficient_stock:${line.fabricId}`);
          }
          tx.update(productRefs[i]!, {
            inStockMeters: FieldValue.increment(-line.meters),
